...
namespace Solid.Dicom.ImageData.Impl
{
    ...
    public class ImageData : IImageData
    {
        ...
        private readonly IImageAttributes _imageAttributes;
        private readonly long _pixelReadingMaskSignbit;
        private readonly long _pixelReadingMaskDatabits;
        private readonly int _pixelReadingBytesPerPixel;
        ...
        internal ImageData(ITracer tracer, IImageAttributes imageAttributes)
        {
            ...
            _pixelReadingMaskSignbit = 0;
            var databits = ImagePixelInfo.BitsStored;
            if (ImagePixelInfo.PixelDataIsSigned)
            {
                _pixelReadingMaskSignbit = 1 << ImagePixelInfo.BitsStored;
                databits--;
            }
            _pixelReadingMaskDatabits = 1;
            for (var idx = 1; idx < databits; idx++)
            {
                _pixelReadingMaskDatabits <<= 1;
                _pixelReadingMaskDatabits |= 1;
            }
            _pixelReadingBytesPerPixel = ImagePixelInfo.BitsAllocated / 8;
        }

        // --- Main dicom information ---
        public string SopInstanceUid => _imageAttributes.SopInstanceUid;
        public int FrameNumber => _imageAttributes.FrameNumber;
        public IImageAttributes ImageAttributes => _imageAttributes;

        // --- ImageInfo instances ---
        public IImageClassInfo ImageClassInfo => _imageAttributes.ImageClassInfo;
        public IImagePlaneInfo ImagePlaneInfo => _imageAttributes.ImagePlaneInfo;
        public IImagePixelInfo ImagePixelInfo => _imageAttributes.ImagePixelInfo;
        public IImageDistortionInfo ImageDistortionInfo => _imageAttributes.ImageDistortionInfo;
        public IImageOrderInfo ImageOrderInfo => _imageAttributes.ImageOrderInfo;

        // --- ImagePlane handling ---
        public bool InvertImagePlaneNormal
        {
            get => _imageAttributes.ImagePlaneInfo.InvertImageNormal;
            set => _imageAttributes.ImagePlaneInfo.InvertImageNormal = value;
        }
        ...
        public void LoadPixelData()
        {
            ...
            var dataSet = _imageAttributes.DataSet;
        ...
        private long ReadPixelValue(int pixelStartIdx, bool invert = false)
        {
            ...
            long pxlbits = 0;
            for (var idx = 0; idx < _pixelReadingBytesPerPixel; idx++)
            {
                // handle little endian vs big endian
                var byteIdx = ImagePixelInfo.PixelDataIsBigEndian ? idx : _pixelReadingBytesPerPixel - 1 - idx;
                pxlbits <<= 8;
            ...
            var pixlValue = pxlbits & _pixelReadingMaskDatabits;
            if ((pxlbits & _pixelReadingMaskSignbit) != 0)
            {
                pixlValue -= _pixelReadingMaskSignbit;
            }
            ...
            if (invert)
            {
                pixlValue = _pixelReadingMaskSignbit != 0 ? -pixlValue : _pixelReadingMaskDatabits - pixlValue;
            }
            ...
            where T : struct, IComparable
        {
            ...
            {
                for (var srcIdx = 0; srcIdx < sourceLength; srcIdx += _pixelReadingBytesPerPixel)
                {
            ...
            for (var srcIdx = 0; srcIdx < sourceLength; srcIdx += _pixelReadingBytesPerPixel)
            {
                ...
                {
                    var isNeg = _pixelReadingMaskSignbit > 0 && pixlValue < 0;
                    if (isNeg)
                    {
                        pixlValue = -pixlValue;
                    }
        ...
        public Bitmap GetPixelsAsBitmap()
        {
            using var trace = _tracer?.CreateScopeTracer();
            var numberOfPixels = ImagePixelInfo.PixelRows * ImagePixelInfo.PixelColumns;
            ConsistencyCheck.EnsureValue(numberOfPixels)
                .IsGreaterOrEqual(1);
